...
class CycleGAN():
    def __init__(self, lr_D=2e-4, lr_G=2e-4, image_shape=(288*1, 368*1, 3*1),
                 date_time_string_addition='_test', image_folder='../videos',
                 speaker1='s1', speaker2='s2', nr_imgs_s1=None, nr_imgs_s2=None,
                 train_ratio_s1=0.6, train_ratio_s2=0.6, batch_size=1,
                 epoch=200):
        ...
        guess_A = D_A(image_A)
        guess_B = D_B(image_B)
        self.D_A = Model(inputs=image_A, outputs=guess_A, name='Discriminator_A_model')
        self.D_B = Model(inputs=image_B, outputs=guess_B, name='Discriminator_B_model')
        ...
        # Use Networks to avoid false keras error about weight descripancies
        self.D_A_static = Network(inputs=image_A, outputs=guess_A, name='Discriminator_A_static_model')
        self.D_B_static = Network(inputs=image_B, outputs=guess_B, name='Discriminator_B_static_model')
        ...
        # Generators
        self.G_A2B = self.modelGenerator(name='Generator_A2B_model')
        self.G_B2A = self.modelGenerator(name='Generator_B2A_model')
        ...
        nr_speaker1_imgs = nr_imgs_s1
        nr_speaker2_imgs = nr_imgs_s2
        speaker1_train_ratio = train_ratio_s1
        speaker2_train_ratio = train_ratio_s2
        ...
        if self.use_data_generator:
            (self.train_data_generator, self.test_data_generator) = load_data.load_data(
                nr_of_channels=self.channels, batch_size=self.batch_size,
                speaker1=speaker1, speaker2=speaker2, video_loc=image_folder,
                nr_speaker1_imgs=nr_speaker1_imgs, nr_speaker2_imgs=nr_speaker2_imgs,
                speaker1_train_ratio=speaker1_train_ratio, speaker2_train_ratio=speaker2_train_ratio,
                generator=True)
        ...
        data = load_data.load_data(nr_of_channels=self.channels,
                                   ...
                                   speaker1=speaker1,
                                   speaker2=speaker2,
                                   video_loc=image_folder,
                                   nr_speaker1_imgs=nr_speaker1_imgs,
                                   nr_speaker2_imgs=nr_speaker2_imgs,
                                   speaker1_train_ratio=speaker1_train_ratio,
                                   speaker2_train_ratio=speaker2_train_ratio)

        self.A_train = data["train1_images"]
        self.B_train = data["train2_images"]
        self.A_test = data["test1_images"]
        self.B_test = data["test2_images"]
        self.testA_image_names = data["sp1_test_names"]
        self.testB_image_names = data["sp2_test_names"]
...
# Training
    def train(self, epochs, batch_size=1, save_interval=1):
        ...
        for epoch in range(1, epochs + 1):
            if self.use_data_generator:
                loop_index = 1
                for images in self.train_data_generator:
                    ...
                    run_training_iteration(loop_index, self.train_data_generator.__len__())
                    ...
                    if loop_index >= self.train_data_generator.__len__():
                        break
...
# Help functions
    ...
    def get_lr_linear_decay_rate(self):
        ...
        if self.use_data_generator:
            max_nr_images = len(self.train_data_generator)
        else:
    ...
    def load_model_and_generate_synthetic_images(self):
        ...
        if response == 'y':
            ...
            def save_image(image, name, domain):
                if self.channels == 1:
                    image = image[:, :, 0]
                imwrite(os.path.join('generate_images', 'synthetic_images', domain, name), image)
...
if __name__ == '__main__':
    GAN = CycleGAN(nr_imgs_s1=2, nr_imgs_s2=2, batch_size=2)
